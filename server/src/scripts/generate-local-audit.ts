...
// We use ts-node or tsx to run this, so we can import from src directly
import { FOLLOWUPS, TCMS, LEADS } from "../../src/lib/mock-data";

async function generateReport() {
  console.log("Fetching data from local mock-data...");
  ...
  try {
    ...
    for (const f of followUps) {
        ...
        const createdAt = f.createdAt ? new Date(f.createdAt).getTime() : (dueAt - 86400000); 
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; }
        ...
        .chart-card { background: var(--card); border: 1px solid var(--border); border-radius: 0.75rem; padding: 1.5rem; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); height: 350px; display: flex; flex-direction: column; }
