...
import { LEADS, TCMS } from "@/lib/mock-data";
...
function AdminImpactCommand() {
  ...
  const stats = useMemo(() => {
    ...
    // Stuck leads: not updated in 3 days, active stages
    const now = Date.now();
    ...
    // Cohorts mapping
    // Active: new, contacted, tour-scheduled, on-tour, tour-done, negotiation, quote-sent
    // Awaiting: intent="warm" but no recent activity (simplified: not-responding-3d)
    // No Response: not-responding-7d
    // Future: intent="cold"
    // Cold: dropped/lost
    // Closed: booked
    ...
    return { totalLeads, toursScheduled, toursDone, bookings, conversion, stuck, cohorts, scoreboard };
  }, [leads, tcms]);
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
            ...
            <div className="space-y-4 max-w-xl">
              <CohortRow icon={Flame} color="text-amber-500" label="Active" count={stats.cohorts.active} total={leads.length} />
              <CohortRow icon={Hourglass} color="text-slate-600 dark:text-slate-400" label="Awaiting" count={stats.cohorts.awaiting} total={leads.length} />
              <CohortRow icon={PhoneOff} color="text-blue-500" label="No Response" count={stats.cohorts.noResponse} total={leads.length} />
              <CohortRow icon={Calendar} color="text-rose-500" label="Future" count={stats.cohorts.future} total={leads.length} />
              <CohortRow icon={Snowflake} color="text-slate-400" label="Cold" count={stats.cohorts.cold} total={leads.length} />
              <CohortRow icon={CheckCircle} color="text-emerald-500" label="Closed" count={stats.cohorts.closed} total={leads.length} />
            </div>
          </div>
